...
for id in `${CURL} -H "Accept:application/json"  -X GET 'https://${appliance_hostname}/isam/application_logs/access_control/runtime'  | jq  -r '.contents[].name' | grep 'trace.*.log'`
do 
   echo $id
