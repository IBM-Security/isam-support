...
while (<>) {
    # print;
    chomp;
    if (/^(\d{4})-(\d{2})-(\d{2})-(\d{2}):(\d{2}):(\d{2})\.(1?\d{3})[-+]\d{2}:00I----- thread\((\d+)\) trace.pdweb.debug.* ----------------- (\w+) ([<=]==[=>]) (\w+) -----------------/) {
	...
	if ($segment eq "PD2Browser") {
	    if (exists $time{$thread}{'Browser2PD'} and exists $time{$thread}{'PD2Browser'}) {
			if ($time{$thread}{'PD2Browser'} - $time{$thread}{'Browser2PD'} > $delay) {
		print $thread, "\t";
		if (exists $time{$thread}{'PD2BackEnd'} and exists $time{$thread}{'BackEnd2PD'}) {
		    print 
