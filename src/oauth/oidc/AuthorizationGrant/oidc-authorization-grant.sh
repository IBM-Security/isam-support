...
AUTHORIZATION_CODE=`./get-authorize.sh $HOST "$CLIENT_ID" "$CLIENT_SECRET" "$REDIR_URI" 2>&1 | grep location | awk -F\= '{ print $3 }'`
echo "$AUTHORIZATION_CODE"

echo ""
echo "Client going back to get tokens (JSON).  This is an unauthenticated request."
./get-token.sh "$HOST" "$CLIENT_ID" "$CLIENT_SECRET" "$AUTHORIZATION_CODE" "$REDIR_URI" | python2 -mjson.tool > token.json
...
#./get-userinfo.sh $HOST $CLIENT_ID $CLIENT_SECRET $ACCESS_TOKEN | python2 -mjson.tool
#
