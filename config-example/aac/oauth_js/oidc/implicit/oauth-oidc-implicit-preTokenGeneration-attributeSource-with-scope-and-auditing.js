...
if (customize_id_token) {
	...
	// Begin scope customizations by jcyarbor@us.ibm.com
	var is_fixed_scope = false;
	var is_cred_scope = false;
	var is_custom_scope = false;
	var is_ldap_scope = false;
	var is_all_scope = false;
	//End of Scope Customization
	
	// Begin auditing customization by jcyarbor@us.ibm.com
	// Store the username for later use
	var username = stsuu.getPrincipalName();
	//End of Auditing Customization
	...
	if (temp_attr != null) {
		for( var scope in temp_attr.getValues()) {
			...
			// Begin scope customizations by jcyarbor@us.ibm.com
			/*
			* The above 'for' loop sets a variable 'scope' that is equal to the current indice of scopes in the 'scope' parameter.
			* Use this to get the current indice and check to see whether it's one of our defined scopes.
			* If the scope matches one of our defined scopes, flip the boolean switch for later processing.
			*/
			
			if (temp_attr.getValues()[scope] == "fixed") {
				is_fixed_scope = true;
			}
			if (temp_attr.getValues()[scope] == "credential") {
				is_cred_scope = true;
			}
			if (temp_attr.getValues()[scope] == "custom") {
				is_custom_scope = true;
			}
			if (temp_attr.getValues()[scope] == "ldap") {
				is_ldap_scope = true;
			}
			if (temp_attr.getValues()[scope] == "all") {
				is_all_scope = true;
			}
			if (temp_attr.getValues()[scope] == "openid") {
				is_oidc_scope = true;
				/* 
				* Comment out the following when using multiple scopes since the
				* order they are checked is not the same as the order they are passed 
				* via the request.
				*/
				//break;
				//End of Scope Customization
	...
	if (populate_id_token || save_cred_attrs) {
		...
		// Beginning of Attribute Source Customization provided by jcyarbor@us.ibm.com
		...
		// End of Attribute Source customization
		
		// Begin processing of scope and auditing customization by jcyarbor@us.ibm.com
		/*
		* Scope processing utlizes the boolean switches from our earlier code on lines 776 - 806
		* The logic is as follows : 
		* 		1. Check to see whether the scope has been presented using the Boolean switch from earlier
		*			- If present, proceed to add the attribute
		*		2. Check to see whether the attribute value is 'null' or an empty string
		*			- If the attribute value is not 'null' or an empty string, add it into the JWT
		*			- If the attribute value is 'null' or an empty string add in a placeholder value
		*				- Audit the value if it's null
		*/
		
		/*
		* First we want to check whether the special scope of 'all' is presented.
		* If this is presented then we'll go ahead and add all the attributes or a placeholder if they are null.
		*/
		if(is_all_scope) {
			// Check to see whether the attribute value is null or an empty string
			if(fixedAttrSrcAttr !=null && fixedAttrSrcAttr != "") {
				// Add the attribute using the type of 'urn:ibm:jwt:claim' and provide an attribut name for the JWT as well 
				...
				// Since the value was missing, we want to audit this so that we can investigate why the attribute source was null
				stsuu.addAttribute(new com.tivoli.am.fim.trustserver.sts.uuser.Attribute("fixedAttribute", "urn:ibm:jwt:claim", "missing"));
				IDMappingExtUtils.logAuditEvent(username,"The Fixed Attribute Source Attribute 'attrSrc-fixedvalue' was missing from the Access Manager Credential", false);
			}
			...
			if(credAttrSrcAttr !=null && credAttrSrcAttr != "") {
				...
				IDMappingExtUtils.logAuditEvent(username,"The Credential Attribute Source Attribute 'attrSrc-credAttr' was missing from the Access Manager Credential", false);
			}
			// Make sure the attribute is not null and if it isn't add it into the STSUU. If the attribute is null, add the attribute as 'missing'
			if(customCredAttrSrcAttr !=null && customCredAttrSrcAttr != "") {
				...
				IDMappingExtUtils.logAuditEvent(username,"The Custom Credential Attribute Source Attribute 'attrSrc-customAttr' was missing from the Access Manager Credential", false);
			}
			// Make sure the attribute is not null and if it isn't add it into the STSUU. If the attribute is null, add the attribute as 'missing'
			if(ldapAttrSrcAttr !=null && ldapAttrSrcAttr != "") {
				...
				IDMappingExtUtils.logAuditEvent(username,"The LDAP Attribute Source Attribute 'attrSrc-ldapAttr' was missing from the Access Manager Credential", false);
			}
		} else {
			if(is_fixed_scope) {
				// Make sure the attribute is not null and if it isn't add it into the STSUU. If the attribute is null, add the attribute as 'missing'
		...
		// End of Scope/Auditing customization
