...
if (customize_id_token) {
	...
	if (request_type == "authorization") {
		...
		if(credMail != null && credMail != ""){
			IDMappingExtUtils.traceString("Pretoken debug :: credMail value : " + credMail);
			stsuu.addContextAttribute(new Attribute("credMail","urn:ibm:names:ITFIM:oidc:claim:value",credMail));
		}

	} else if (request_type == "access_token") {
		IDMappingExtUtils.traceString("Pretoken debug :: : request_type = access_token");
		...
		if (code != null) {
			...
			if (token != null) {
				...
				IDMappingExtUtils.traceString("Pretoken debug :: : request_type = access_token : state_id value : " + state_id);
			}
		}
	}
